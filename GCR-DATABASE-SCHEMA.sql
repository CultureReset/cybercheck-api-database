-- ========================================
-- GULF COAST RADAR — COMPREHENSIVE BUSINESS DATABASE SCHEMA
-- All tables needed for rich business data storage
-- ========================================
...
-- ========================================
-- CONTENT SECTIONS (Flexible structure for any type of content)
-- ========================================
CREATE TABLE IF NOT EXISTS entity_sections (
    ...
    section_type TEXT NOT NULL,  -- 'menu', 'drinks', 'happy_hour', 'events', 'about', etc.
    section_note TEXT,
    icon TEXT,
    sort_order INTEGER DEFAULT 0,
    created_at TIMESTAMP DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS entity_photos (
    ...
    caption TEXT,
    photo_type TEXT,  -- 'hero', 'menu', 'interior', 'food', 'event', etc.
    credit TEXT,
    sort_order INTEGER DEFAULT 0,
    created_at TIMESTAMP DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS entity_events (
    ...
    start_time TIME,
    end_time TIME,
    event_type TEXT,  -- 'live_music', 'special_dinner', 'happy_hour', etc.
    description TEXT,
    venue_location TEXT,
...
CREATE TABLE IF NOT EXISTS entity_specials (
    ...
    special_type TEXT,  -- 'happy_hour', 'promotion', 'seasonal', 'limited_time'
    description TEXT,
    discount_type TEXT,  -- 'percentage', 'fixed_amount', 'bogo', 'discount_price'
...
-- ========================================
-- LEGACY MENU TABLES (Maintain compatibility)
-- ========================================
...
-- ========================================
-- PRODUCTS & SHOPPING (For retail, bookings, etc.)
-- ========================================
...
-- ========================================
CREATE OR REPLACE VIEW businesses_with_menu AS
SELECT
    e.id, e.slug, e.name,
    COUNT(DISTINCT CASE WHEN es.section_type = 'menu' THEN es.id END) as menu_section_count,
    COUNT(DISTINCT CASE WHEN si.section_id IN (SELECT id FROM entity_sections WHERE section_type = 'menu') THEN si.id END) as menu_item_count
FROM entity e
...
CREATE OR REPLACE VIEW businesses_with_events AS
SELECT
    e.id, e.slug, e.name,
    COUNT(ee.id) as upcoming_event_count,
    COUNT(CASE WHEN ee.event_date >= CURRENT_DATE THEN 1 END) as upcoming_events
FROM entity e
LEFT JOIN entity_events ee ON e.id = ee.entity_id
GROUP BY e.id, e.slug, e.name;

CREATE OR REPLACE VIEW businesses_with_specials AS
SELECT
    e.id, e.slug, e.name,
    COUNT(es.id) as active_special_count
FROM entity e
LEFT JOIN entity_specials es ON e.id = es.entity_id AND es.is_active = true
GROUP BY e.id, e.slug, e.name;
