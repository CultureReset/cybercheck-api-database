#!/usr/bin/env node
/**
 * COPY TABLE SCHEMAS: OLD DB → NEW GCR DB
 * Creates all table structures from old DB in new DB
 */
...
async function getTableSchema(tableName) {
  try {
    ...
    if (!data || data.length === 0) {
      // Try to get column info another way - select with head
      const { error: headError } = await old.from(tableName).select('*', { head: true });
      if (headError) return null;

      // If we can select with head but no data, columns exist but table is empty
      // We'll still include it
      return { columns: [] };
    }
...
async function tableExists(tableName) {
  try {
    const { error } = await gcr.from(tableName).select('*', { head: true, limit: 0 });
    return !error;
  } catch (e) {
    return false;
  }
...
async function main() {
  ...
  for (const tableName of ALL_TABLES) {
    ...
    if (exists) {
      console.log(`✓  ${tableName.padEnd(35)} - Already exists`);
      skipped++;
    } else {
      const schema = await getTableSchema(tableName);
      if (schema) {
        console.log(`📍 ${tableName.padEnd(35)} - Needs creation (${schema.columns.length || 0} columns)`);
        missing.push(tableName);
      } else {
        console.log(`❌ ${tableName.padEnd(35)} - Cannot read schema`);
      }
    }
  }
  ...
  console.log(`Need creation: ${missing.length}`);
  console.log(`Cannot read:   ${ALL_TABLES.length - skipped - missing.length}\n`);

  if (missing.length > 0) {
    console.log('📍 TABLES THAT NEED MANUAL CREATION:');
    missing.forEach(t => console.log(`   - ${t}`));
    console.log('\n⚠️  These tables must be created manually via SQL migration.\n');
    console.log('NEXT STEP: Create these tables in GCR DB, then run full-transfer.js\n');
  } else {
    console.log('✅ All tables exist! Ready to run full-transfer.js\n');
  }
}
